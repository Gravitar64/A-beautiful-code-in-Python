...
def würfeln(sp, pkt, zug=0):
  while True:
    ...
    print(
      f'Spieler {sp}: {wurf} gewürfelt, Punkte Zug = {zug:<2}, Punkte Gesamt = {pkt+zug} (Max = {max(punkte)})')
    if zug and input('Weiterwürfeln im Zug? (j/n) ').lower() == 'j':
      continue
    return zug
...
while True:
  ...
  if not anz_spieler.isdigit():
    continue
  if 2 <= (spieler := int(anz_spieler)) <= MA_SP:
    break
