...
def draw(pos1, pos2, m):
  pg.draw.line(screen, (255,255,255), pos1, pos2, 3)
  pg.draw.circle(screen, (255,0,0), pos2, m)
...
while weitermachen:
  ...
  θ1_2 = -g*(2*m1 + m2)*sin(θ1) - m2 * g * sin(θ1 - 2*θ2) - 2*sin(θ1 - θ2)*m2* (θ2_1**2*L2 + θ1_1**2*L1*cos(θ1 - θ2))
  θ1_2 = θ1_2 / (L1*(2*m1 + m2 - m2*cos(2*θ1 - 2*θ2)))

  θ2_2 = 2 * sin(θ1 - θ2) * (θ1_1**2*L1* (m1 + m2) + g*(m1 + m2)* cos(θ1) + θ2_1**2*L2*m2*cos(θ1 - θ2))
