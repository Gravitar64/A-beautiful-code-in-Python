...
def ermittle_stich_gewinner(stich):
  ...
  höchste_karte = höchste_trumpf_karte = -1
  for name, (wert, farbe) in stich:
    if wert == 14: continue
    if wert == 15: return name
    if farbe == trumpf and wert > höchste_trumpf_karte:
      höchste_trumpf_karte = wert
      stich_gewinner = name  
    if wert > höchste_karte and farbe == aktuelle_farbe and höchste_trumpf_karte == -1:
      höchste_karte = wert
      stich_gewinner = name
