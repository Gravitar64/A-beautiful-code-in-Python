...
spielfeld = {(rnd.randint(feld_von, feld_bis), rnd.randint(feld_von, feld_bis), rnd.randint(feld_von, feld_bis)) for _ in range(anz)}
animate_frame = 8
...
for i in range(200):
    ...
    if not new and not dead: break
    for zelle in new|dead:
        if zelle not in cubes: continue
        ob = cubes[zelle]
        if zelle in new:
